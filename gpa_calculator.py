...
class Application:
    ...
    def __init__(self):
        ...
        self.add_course_button = ttk.Button(self.root, text="Add Course", command=self.add_course)
        self.add_course_button.pack(side="top")

        self.compute_GPA_button = ttk.Button(self.root, text="Compute GPA",
                                             command=lambda: self.compute_gpa(self.selected_calculations))
        self.compute_GPA_button.pack(side="top")

        self.load()

        self.actionLabel = ttk.Label(self.course_frame, text="Actions:")
        self.actionLabel.grid(row=len(self.courses_GUI) + 1, column=0)
        ...
        self.ytd_checkbox.grid(row=len(self.courses_GUI) + 1, column=1)
        ...
        self.sem_checkbox.grid(row=len(self.courses_GUI) + 1, column=3)

        [self.selected_calculations.append(item) for item in [self.ytd_checked, self.sem_checked]]
        self.compute_widgets.extend([(self.actionLabel, self.ytd_checkbox, self.sem_checkbox)])
    ...
    def compute_gpa(self, options):
        ...
        for option in options:
            if option.get() == 1:
                ...
                calculated_gpa.grid(row=self.compute_widgets[-1][-1].grid_info()["row"] + 1, column=1)
    ...
    def save(self):
        ...
        with open("course_data.csv", "w") as csv_file:
            field_names = ["name", "level", "grade", "credits"]
            csv_writer = csv.DictWriter(csv_file, fieldnames=field_names)
            csv_writer.writeheader()
            for course in self.courses_GUI:
                csv_writer.writerow(course.read())
    ...
    def load(self):
        with open("course_data.csv", "r") as csv_file:
            ...
            for line in lines:
                self.add_course()
                current_course = self.courses_GUI[lines.index(line)]
