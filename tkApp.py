...
class tkCourseGUI(core.CourseGUI):
    ...
    def __init__(self, row, frame, del_button_command):
        ...
        core.CourseGUI.__init__(self, entry_widget_values,
                                entry_widgets, Grid.grid_forget)
...
class tkTab(core.Tab):
    ...
    def __init__(self, notebook, year):
        ...
        self.actionLabel = ttk.Label(
            self.course_widget_container, text="Actions:")
        ...
        self.add_course_button = ttk.Button(
            self.course_widget_container, text="Add Course", command=self.add_course)
        self.add_course_button.grid(
            row=len(self.courses_GUI) + 2, column=7, sticky="E")
        ...
        self.compute_GPA_button.grid(
            row=len(self.courses_GUI) + 2, column=9, sticky="W")
    ...
    def create_compute_widgets(self):
        ...
        options = [calc.get() for calc in self.selected_calculations]
        ...
        for i, option in enumerate(options):
            if option == 1:
                ...
                label = ttk.Label(
                    self.course_widget_container, text=label_texts[i])
                label.grid(
                    row=self.compute_widgets[-1][-1].grid_info()["row"] + 2, column=0)
                calculated_gpa = ttk.Label(
                    self.course_widget_container, text=str(corresponding_value[i]))
                calculated_gpa.grid(
                    row=self.compute_widgets[-1][-1].grid_info()["row"] + 2, column=1)
                self.compute_widgets.append((label, calculated_gpa))

    def compute_gpa(self, options):
        # Computes the GPA from the tabs courses on click
        try:
...
class tkApp(core.Application):
    ...
    def __init__(self):
        ...
        self.help_menu.add_command(
            label="Directions", command=lambda: self.pop_up("Directions", self.directions))
        self.help_menu.add_command(
            label="About", command=lambda: self.pop_up("About", self.about_message))
